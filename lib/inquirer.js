...
import { default as Radio } from './prompts/radio.js';
...
export function createPromptModule(opt) {
  const promptModule = function (questions, answers) {
    let uiInstance;
    try {
      uiInstance = new Prompt(promptModule.prompts, opt, createCallback);
    } catch (error) {
  ...
  promptModule.restoreDefaultPrompts = function () {
    ...
    this.registerPrompt('radio', Radio);
...
const cancelString = commonStrings.cancelString;
...
function setCreationCallback(creationCallback) {
  creationCallback(prompt)
}
